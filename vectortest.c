...
static void TestAppend(CVector *alphabet)
{
  char ch;
  int i;

  for (ch = 'A'; ch <= 'Z'; ch++)   //  Start with letters of alphabet
    CVectorAppend(alphabet, &ch);
  fprintf(stdout, "First, here is the alphabet: ");
  CVectorMap(alphabet, PrintChar, stdout);
...
static void TestSearch(CVector *v, char ch)
{
  int foundSorted, foundNot;

  foundSorted = CVectorSearch(v, &ch, CompareChar, 0, true); // Test sorted
  foundNot = CVectorSearch(v, &ch, CompareChar, 0, false);   // Not sorted
...
static void TestSortSearch(CVector *alphabet)
{
  CVectorSort(alphabet, CompareChar);	 // Sort into order again
  fprintf(stdout, "\nAfter sorting: ");
  CVectorMap(alphabet, PrintChar, stdout);
...
static void TestAt(CVector *alphabet)
{
  int i;

  for (i = 0; i < CVectorCount(alphabet); i += 2) { // Lowercase every other
    char *elem = (char *) CVectorNth(alphabet, i);
    *elem = tolower(*elem);
  }
...
static void TestInsertDelete(CVector *alphabet)
{
  char ch = '-';
  int i;

  for (i = 3; i < CVectorCount(alphabet); i += 4) // Insert dash every 4th char
    CVectorInsert(alphabet, &ch, i);
  fprintf(stdout, "\nAfter insert dashes: ");
  CVectorMap(alphabet, PrintChar, stdout);

  for (i = 3; i < CVectorCount(alphabet); i += 3) // Delete every 4th char
    CVectorRemove(alphabet, i);
  fprintf(stdout, "\nAfter deleting dashes: ");
  CVectorMap(alphabet, PrintChar, stdout);

  ch = '!';
  CVectorInsert(alphabet, &ch, CVectorCount(alphabet));
  CVectorRemove(alphabet, CVectorCount(alphabet) - 1);
...
static void TestReplace(CVector *alphabet)
{
  int found = 0;
  char toFind = 's', toReplace = '*';

  while (found < CVectorCount(alphabet)) {
    found = CVectorSearch(alphabet, &toFind, CompareChar, found, false);
    if (found == -1) break;
    CVectorReplace(alphabet, &toReplace, found);
  }
...
static void InsertPermutationOfNumbers(CVector *numbers, long n, long d)
{
  long k;
  long residue;
  ...
  for (k = 0; k < d; k++) {
    ...
    CVectorAppend(numbers, &residue);
  }

  assert(CVectorCount(numbers) == d);
...
static void SortPermutation(CVector *vectorToSort)
{
  long residue, embeddedLong;
  CVector *sortedVector;
  fprintf(stdout, "Sorting all of those numbers. ");
  fflush(stdout);
  CVectorSort(vectorToSort, LongCompare);
  fprintf(stdout, "[Done]\n");
  fflush(stdout);
  fprintf(stdout, "Confirming everything was properly sorted. ");
  fflush(stdout);
  sortedVector = vectorToSort; // need better name now that it's sorted...
  for (residue = 0; residue < CVectorCount(sortedVector); residue++) {
    embeddedLong = *(const long *) CVectorNth(sortedVector, residue);
...
static void DeleteEverythingVerySlowly(CVector *numbers)
{
  ...
  fflush(stdout);
  largestOriginalNumber = *(long *)CVectorNth(numbers, CVectorCount(numbers) - 1);
  while (CVectorCount(numbers) >= 100) {
    CVectorRemove(numbers, CVectorCount(numbers) - 100);
    assert(largestOriginalNumber == *(long *)CVectorNth(numbers, CVectorCount(numbers) -1));
  }
  fprintf(stdout, "\t[Okay, almost done... deleting the last 100 elements... ");
  fflush(stdout);
  while (CVectorCount(numbers) > 0) CVectorRemove(numbers, 0);
...
static void ChallengingTest()
{
  ...
  CVector *lotsOfNumbers;
  fprintf(stdout, "\n\n------------------------- Starting the more advanced tests...\n");
  lotsOfNumbers = CVectorCreate(sizeof(long), 4, NULL);
...
static void MemoryTest()
{
  ...
  for (i = 0; i < kNumQuestionWords; i++) {
    ...
    CVectorInsert(questionWords, &questionWord, 0);  // why the ampersand? isn't questionWord already a pointer?
  }

  fprintf(stdout, "Mapping over the char * vector (ask yourself: why are char **'s passed to PrintString?!!)\n");
  CVectorMap(questionWords, PrintString, stdout);
  fprintf(stdout, "Finally, destroying the char * vector.\n");
  CVectorDispose(questionWords);
}
